...
<body>
    ...
    <script>
        alert('Sukses Aktifkan User');
    </script>
    @endif
    @if(session()->has('nonaktif-status'))
    <script>
        alert('Sukses Nonaktifkan User');
    </script>
    @endif
    ...
    <div class="flex flex-row w-full" style="min-height:100vh">
        ...
        <div class="bg-gray-200 w-full px-4 py-12">
            <a href="/dashboard/tambah-user"><button class="btn btn-primary">Tambah User</button></a>
      <br>     <br>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                   @if($item->active == 0)
                   <a href="/dashboard/manajemen-user/active/aktif/{{$item->id}}"> <button class="btn-sm btn-success">Active User</button> </a>
                   @else
                   <a href="/dashboard/manajemen-user/active/nonaktif/{{$item->id}}"> <button class="btn-sm btn-secondary">Non Aktifkan</button>
                   @endif
                   <a href="/dashboard/manajemen-user/edit/{{$item->id}}"> <button class="bg-blue-400 px-3 py-1 text-white">Edit User</button> </a>
                   <a href="/dashboard/manajemen-user/hapus/{{$item->id}}" onclick="return confirm('Yakin Untuk Delete User?')">   <button class="btn-sm btn-danger">Delete User</button></a> 
                </td>
            </tr>
            @endforeach
        </tbody>
